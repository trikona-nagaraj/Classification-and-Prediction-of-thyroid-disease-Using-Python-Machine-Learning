...
class Model_Finder:
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            self.prediction_xgboost = self.xgboost.predict_proba(test_x)  # prediction using the Random Forest Algorithm

            if len(test_y.unique()) == 1:  # if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                self.xgboost_score = accuracy_score((test_y), self.prediction_xgboost)
                self.logger_object.log(self.file_object, 'Accuracy for Xg Boost:' + str(self.xgboost_score))
            else:
                self.xgboost_score = roc_auc_score((test_y), self.prediction_xgboost, multi_class='ovr')  # AUC for XGboost
                self.logger_object.log(self.file_object, 'AUC for Xg Boost:' + str(self.xgboost_score))


            #comparing the  models

            if(self.random_forest_score <  self.knn_score  and self.xgboost_score < self.knn_score):
                return 'KNN',self.knn

            elif (self.random_forest_score <  self.xgboost_score  and self.knn_score < self.xgboost_score):
